...
- **多选复选框替换为来源 Agent 圆形徽标** — 列表项前的原生 checkbox 改为圆形 brand 色底 + 白色 logo/首字母（Claude / Gemini / Cursor / DeepSeek / Grok 用真实品牌标，其余无公开标的用缩写）；未选中只显徽标，选中时叠加半透明主题强调色遮罩 + 带环 tick（环/勾随主题文字色），保留 role=checkbox 与键盘切换的可访问性；来源文本框（itemMeta 里的 format 徽章）不再显示。
