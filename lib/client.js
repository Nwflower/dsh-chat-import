...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
     ...
     * `flex:none; width:calc(100% + 4px)`）与 @kenz1117/dsh-ui-usage-billing 的
     * 计费卡（[data-testid='billing-trigger']，`flex:auto; width:100%`）都会占满
     * 不换行的 footer flex 行，把同槽其它条目挤成窄条/竖排文字（issue #35）。
     * 返回最靠上的一个可见占用者的视口坐标（{top,left,width,height}），无则 null
     * —— 用于把本按钮浮到其上方独占一行、左缘与宽度对齐，与占用者形成并行两行布局。 */
    const footerOccupantRect = () => {
      if (typeof document === "undefined") return null;
      let best = null;
      for (const sel of ["[data-cordis-badge]", ".dshMarketLauncher", "[data-testid='billing-trigger']"]) {
