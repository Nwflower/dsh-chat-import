...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    /** 触发按钮：三种布局模式自适应。
     * 1) 槽容器 wrap（本插件在有整行占用者时主动注入，或 dsh-tokenledger 等
     *    插件注入）→ 行内整宽条目各占一行、order 决定堆叠，本按钮以整宽行
     *    落在 cordis 徽标下方、设置按钮上方，无任何浮层；
     * 2) 容器 nowrap 且行内出现整行占用者（官方 cordis 徽标 / 内置插件市场
     *    launcher）→ 本插件把容器换成 wrap（见 ImportButton 的 effect），只有
     *    容器无法定位时才回退 fixed 浮层锚到占用者正上方；
     * 3) 容器 nowrap 且无占用者 → 行内自适应：row 容器 flex: 1 1 auto 与同
     *    槽其它入口共享一行（issue #31）；column 容器（usage-stats 纵排）
     *    保持全宽、高度随内容（issue #25）。
     * footerActions 默认是不换行的 flex 行；cordis 徽标 `width: calc(100% + 4px)`、
     * 市场 launcher `flex:none; width:calc(100% + 4px)` 均不可收缩、占满整行，
     * nowrap 时会把同槽其它条目挤出容器并被侧边栏 overflow:hidden 裁剪、或挤成
     * 竖排窄条（实测）。fixed 浮层虽能独占一行，却脱离文档流、压住会话列表末尾行
     * （列表长时重叠），故优先用换行把按钮落回文档流、让 footArea 高度随内容增长。
     * 样式对齐设置按钮（透明底、12px 圆角、16px 图标 + 文字、悬停浅底），图标用
     * 插件 logo；rail（wide=false）态只显图标。
     */
    /** 检测 footer 行内的「整行占用者」：官方 cordis 徽标（[data-cordis-badge]）、
     * 内置插件市场 launcher（.dshMarketLauncher）、@kenz1117/dsh-ui-usage-billing 的
     * 计费卡（[data-testid='billing-trigger']）与 dsh-cost-meter 的侧栏费用卡
     * （.cm-footer-stack）都会占满不换行的 footer flex 行，把同槽其它条目挤成窄条/
     * 竖排文字。返回最靠上的一个可见占用者的视口坐标（{top,left,width,height}），
     * 无则 null —— 用于判定是否给容器换行（有占用者 → wrap），并作为 fixed 浮层
     * 兜底（容器无法定位时）的锚点。 */
    ...
    function ImportButton({ wide }) {
      ...
      // 槽容器已 wrap（本插件在有占用者时注入，或 tokenledger 等插件注入）→ 行内
      // 整宽条目各占一行、order 决定上下（本按钮 0 落在 cordis 徽标下方）；
      // nowrap 且有整行占用者（cordis 徽标 / 插件市场 launcher）→ effect 主动把容器
      // 换成 wrap，容器无法定位才回退 fixed 浮层锚到占用者正上方；
      ...
      useEffect(() => {
        ...
        const check = () => {
          ...
          // 有整行占用者（cordis 徽标等）时给 footerActions 换行，让本按钮以整宽行落到
          // 占用者下方、与「插件/设置」纵向堆叠，避免 fixed 浮层压住会话列表末尾行
          // （列表长时重叠）；无占用者不动容器（保持 issue #31 共享一行），已 wrap
          // （tokenledger 等注入）不覆盖。仅在宽态（data-dsh-sidebar-wide="true"）换行：
          // rail 态无会话列表、浮层不重叠，且 rail 容器 width:auto，强行 wrap 会影响
          // 56px 纵栏里图标堆叠的居中。写入 inline style 会触发 MutationObserver 再跑
          // check，但 `!== "wrap"` 守卫使其不重复赋值、不形成循环；卸载时还原容器原始值。
          const root = document.querySelector("[data-dsh-sidebar-root]");
          const wideNow = !root || root.getAttribute("data-dsh-sidebar-wide") !== "false";
          if (wideNow && container && occupant && container.style.flexWrap !== "wrap") {
