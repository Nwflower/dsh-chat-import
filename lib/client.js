...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const PAGE_SIZE = 50;
    // 流式缓冲的时间倒序排序（对齐服务端 discoverSessions 的 lastActiveAt 降序）
    ...
    function ImportSettingsSection() {
      ...
      const applyPref = (next) => {
        ...
        fetch("/api-import/prefs", {
          ...
          .catch((err) => { setState((s) => ({ ...s, saving: false, error: t("error.route") })); });
      };
      ...
      useEffect(() => {
        ...
        (async () => {
          ...
          while (!cancelled && !done && !failed) {
            ...
            if (batch.length > 0) {
              setItems((prev) => prev.concat(batch).sort(byTimeDesc));
            }
    ...
    // locale 是晚挂载服务（dsh-client-locale 自身依赖 connection/remote/settingsScope），
    // 声明进 inject 让 apply 期 ctx.get('locale') 就绪（面板 i18n + 字典注册）。
    // settingsScope 仍可能缺席（无设置页的 profile），故在 slots.inject 回调里 resolve。
    ...
    function apply(ctx) {
      ...
      //（settings.plugins.tab 是「插件」分区内部的子页，仅适合分区内页面，不适合作
      // 插件设置入口；此前挂该槽在部分宿主版本下点不开）。该槽由 ui-settings-general
      // 声明，而 settingsScope（依赖 connection/remote）晚于本插件 apply 期；
      // settingsScope 是可选服务（dsh-client-ui-settings），缺席时不注册；slots.inject
      // 惰性挂到槽被声明时，槽不存在（无设置页的 profile）则回调永不执行、不报错。
      // 必须在回调内 resolve settingsScope——apply 期 ctx.get('settingsScope') 拿到
      // undefined 会让整段跳过、分区永不注册。label 用 thunk 跟随语言切换（同
      // agent-presets / plugins 等官方分区写法）。
      ctx.slots.inject("settings.section", () => {
        ...
        return ctx.slots.register(
          { name: "settings.section", id: "chat-import", order: 21, label: () => t("settings.tab"), locale: LOCALE_NS, inject: () => ({ scope }) },
