  ...
  factory: (require) => {
    ...
    const DICT = {
      zh: {
        ...
        "import.all": "搬空全部 ({n})",
        "import.all.title": "导入当前扫描结果中所有未导入会话（跨页，非仅当前页）",
        "import.filter": "搬空当前筛选 ({n})",
        "import.filter.title": "仅导入当前来源、工作区与搜索条件下的未导入会话",
        "import.filter.confirm.title": "确认搬空",
        "import.filter.confirm": "当前筛选 {n} 条未导入会话，确认全部搬入 DSH？会话将挂到真实项目目录，而非 transcript UUID 目录。",
        "import.filter.confirm.ok": "确认搬空",
        "import.filter.confirm.cancel": "取消",
        "refresh.imported": "刷新已导入 ({n})",
        "refresh.imported.title": "按当前来源、工作区与搜索条件，重新转换并覆盖已导入会话（不新建副本）",
        "refresh.imported.confirm.title": "确认刷新",
        "refresh.imported.confirm": "将重新转换并覆盖 {n} 条已导入会话的正文与标题，会话 ID 不变。确定继续？",
        "refresh.imported.confirm.ok": "确认刷新",
        "refresh.imported.confirm.cancel": "取消",
        "pageSize": "每页",
        "search": "搜索",
        "previous": "上一页",
        "next": "下一页",
        ...
        "settings.injectTools.title": "将本插件工具显式注入对话上下文",
        "settings.injectTools.description": "建议关闭，可节省约 5k 上下文。关闭后不再向对话内的 Agent 提供本插件工具，仍可通过 GUI「导入会话」面板完成转换。",
        "settings.tab": "会话导入",
      },
      en: {
        ...
        "import.all": "Import all ({n})",
        "import.all.title": "Import every not-yet-imported session in the current scan (all pages, not just this page)",
        "import.filter": "Import filtered ({n})",
        "import.filter.title": "Import only not-yet-imported sessions matching source, workspace, and search",
        "import.filter.confirm.title": "Confirm import",
        "import.filter.confirm": "Import {n} not-yet-imported sessions matching the current filters? They attach to the real project folder, not a transcript UUID directory.",
        "import.filter.confirm.ok": "Import",
        "import.filter.confirm.cancel": "Cancel",
        "refresh.imported": "Refresh imported ({n})",
        "refresh.imported.title": "Re-convert and overwrite imported sessions matching current filters (same session id, no duplicates)",
        "refresh.imported.confirm.title": "Confirm refresh",
        "refresh.imported.confirm": "Re-convert and overwrite {n} imported sessions (titles and bodies). Session IDs stay the same. Continue?",
        "refresh.imported.confirm.ok": "Refresh",
        "refresh.imported.confirm.cancel": "Cancel",
        "pageSize": "Per page",
        "search": "Search",
        "previous": "Previous",
        "next": "Next",
        ...
        "settings.injectTools.title": "Explicitly inject this plugin's tools into the conversation context",
        "settings.injectTools.description": "Recommended off to save about 5k of context. When off, this plugin's tools are not provided to the agent in-conversation; you can still convert via the Import Sessions panel.",
    ...
    const FORMAT_SOURCE = {
      ...
      gemini: "gemini", reasonix: "reasonix", opencode: "opencode", mimocode: "mimocode", kilocode: "kilocode", zcode: "zcode",
      grokbuild: "grokbuild", openclaw: "openclaw", pi: "pi", hermes: "hermes",
      kimi: "kimi", qoder: "qoder", workbuddy: "workbuddy", qwen: "qwen", dsh: "dsh",
    };
    // 来源展示名（产品名不翻译；qwen = 千问办公）
    const SOURCE_LABELS = {
      "claude-code": "Claude Code", codex: "Codex", chatgpt: "ChatGPT", cursor: "Cursor",
      gemini: "Gemini CLI", reasonix: "Reasonix", opencode: "OpenCode", mimocode: "MimoCode",
      kilocode: "Kilo Code",
      zcode: "ZCode", grokbuild: "Grok Build", openclaw: "OpenClaw", pi: "Pi Coding Agent",
    ...
    const makeStyles = (C) => ({
      ...
      searchBtn: {
        flex: "none", background: C.accent, color: C.accentForeground, border: "none", borderRadius: "6px",
      ...
      primaryBtn: {
        flex: "1", background: C.accent, color: C.accentForeground, border: "none", borderRadius: "6px",
      ...
      importedTag: { flex: "none", fontSize: "11px", color: C.success, marginTop: "2px", whiteSpace: "nowrap" },
      syncBtn: {
    ...
    function ImportSettingsSection() {
      ...
      const [state, setState] = useState({ importSystemPrompt: false, injectTools: false, saving: false, error: null });
      const readPrefs = (data) => ({
        importSystemPrompt: !!(data && data.value && data.value.importSystemPrompt),
        injectTools: !!(data && data.value && data.value.injectTools),
      });
      const load = () => {
      ...
      return React.createElement("div", { style: { padding: "20px 24px", display: "flex", flexDirection: "column", gap: "16px", maxWidth: "640px" } },
        ...
        toggleCard(t("settings.injectTools.title"), t("settings.injectTools.description"), state.injectTools, "injectTools"),
      ...
      return React.createElement("div", { style: { display: "flex", flexDirection: "column", gap: "16px" } },
        ...
        React.createElement("div", { style: { display: "flex", alignItems: "center", gap: "12px" } },
          React.createElement("button", {
            style: {
              background: colors.accent, color: colors.accentForeground, border: "none", borderRadius: "6px",
    ...
    function ImportTabContent({ onClose }) {
      ...
      const tabBtn = (id, label) => React.createElement("button", {
        ...
        onClick: () => setTab(id),
        style: {
          flex: "1", padding: "8px 0", border: "none", cursor: "pointer", fontSize: "12.5px", fontWeight: 600,
     ...
     *  tab（service 在 apply 里 registerTab 时闭包）。面板展开由 openTab 的 CONTENT
     *  open（path seed）负责，组件自身不碰 panel 状态。 */
    ...
    function HistoryPanel({ embedded }) {
      ...
      const confirmDialog = confirm && React.createElement("div", {
        ...
        React.createElement("div", {
          ...
          React.createElement("div", { style: { fontSize: "12.5px", color: colors.dim, marginBottom: "14px", lineHeight: 1.5 } },
            confirm.kind === "all"
      ...
      const body = React.createElement(React.Fragment, null,
        ...
        !loading && entries.length > 0 && React.createElement("div", { style: style.list },
          entries.map((e) => React.createElement("div", {
            ...
            React.createElement("div", { style: { fontSize: "12.5px", fontWeight: 600, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" } },
              e.title || t("noTitle")),
            ...
            React.createElement("div", { style: style.itemMeta },
              ...
              React.createElement("span", null, fmtTime(e.importedAt) || t("timeUnknown")),
      ...
      return React.createElement("div", { ref: rootRef, style: { position: "relative", flex: 1, minWidth: 0 }, title },
        React.createElement("button", {
          ...
          style: {
            ...
            color: colors.text, borderRadius: "6px", padding: "6px 8px", fontSize: "13px",
          ...
          React.createElement("span", { style: { color: colors.dim, fontSize: "10px", flex: "none" } }, open ? "▲" : "▼")),
        open && React.createElement("div", {
          ...
          React.createElement("input", {
            ...
            style: {
              ...
              padding: "7px 10px", fontSize: "12.5px", outline: "none",
            },
          }),
          React.createElement("div", { style: { maxHeight: "240px", overflowY: "auto" } },
            ...
            shown.map((o) => React.createElement("div", {
              ...
              style: {
                padding: "7px 10px", fontSize: "12.5px", cursor: "pointer", color: colors.text,
    ...
    function DiscoveryPanel({ onClose, embedded }) {
      ...
      const [confirmFilterImport, setConfirmFilterImport] = useState(null); // { n, items }
      const [confirmRefreshImported, setConfirmRefreshImported] = useState(null); // { n, items }
      ...
      // 可搬空：扫描完成且未导入/未归档的条目（跨页全量，非仅当前页）
      const importableAll = stream.done
        ? importableSessions(items, "")
        : [];
      ...
      const filterImportDialog = confirmFilterImport && React.createElement("div", {
        style: {
          position: "absolute", inset: 0, background: "rgba(0,0,0,.5)", zIndex: 2,
          display: "flex", alignItems: "center", justifyContent: "center", padding: "16px",
        },
        ...
        React.createElement("div", {
          ...
          React.createElement("div", { style: { fontWeight: 600, marginBottom: "8px" } }, t("import.filter.confirm.title")),
          React.createElement("div", { style: { fontSize: "12.5px", color: colors.dim, marginBottom: "14px", lineHeight: 1.5 } },
            t("import.filter.confirm", { n: confirmFilterImport.n })),
          React.createElement("div", { style: { display: "flex", gap: "8px", justifyContent: "flex-end" } },
            React.createElement("button", {
              style: style.toolBtn, disabled: importing,
              onClick: () => setConfirmFilterImport(null),
            }, t("import.filter.confirm.cancel")),
            React.createElement("button", {
              style: { ...style.primaryBtn, flex: "none", padding: "6px 14px" },
              disabled: importing,
              onClick: () => {
                const batch = confirmFilterImport.items;
                setConfirmFilterImport(null);
                doImport(batch);
              },
            }, t("import.filter.confirm.ok")))));

      const refreshImportDialog = confirmRefreshImported && React.createElement("div", {
        style: {
          position: "absolute", inset: 0, background: "rgba(0,0,0,.5)", zIndex: 2,
          display: "flex", alignItems: "center", justifyContent: "center", padding: "16px",
        },
        ...
        React.createElement("div", {
          ...
          React.createElement("div", { style: { fontWeight: 600, marginBottom: "8px" } }, t("refresh.imported.confirm.title")),
          React.createElement("div", { style: { fontSize: "12.5px", color: colors.dim, marginBottom: "14px", lineHeight: 1.5 } },
            t("refresh.imported.confirm", { n: confirmRefreshImported.n })),
          React.createElement("div", { style: { display: "flex", gap: "8px", justifyContent: "flex-end" } },
            React.createElement("button", {
              style: style.toolBtn, disabled: importing,
              onClick: () => setConfirmRefreshImported(null),
            }, t("refresh.imported.confirm.cancel")),
            React.createElement("button", {
              style: { ...style.primaryBtn, flex: "none", padding: "6px 14px" },
              disabled: importing,
              onClick: () => {
                const batch = confirmRefreshImported.items;
                setConfirmRefreshImported(null);
                doImport(batch, { replace: true });
              },
            }, t("refresh.imported.confirm.ok")))));
      ...
      const body = React.createElement(React.Fragment, null,
          ...
          React.createElement("div", { style: style.toolbar },
            React.createElement("button", { style: style.toolBtn, onClick: toggleAll, disabled: filteredItems.length === 0 || importing }, allSelected ? t("deselectAll") : t("selectAll")),
            ...
            React.createElement("button", {
              style: style.toolBtn, onClick: () => setEpoch((n) => n + 1), disabled: importing, title: t("refresh.title"),
            }, t("refresh")),
          ...
          React.createElement("div", { style: style.importBar },
            ...
            React.createElement("button", {
              style: {
                ...style.toolBtn, flex: "none", fontWeight: 600,
                opacity: importableFiltered.length === 0 || importing || !stream.done ? 0.55 : 1,
                borderColor: colors.accent, color: colors.accent,
              },
              disabled: importableFiltered.length === 0 || importing || !stream.done,
              title: t("import.filter.title"),
              onClick: () => setConfirmFilterImport({
                n: importableFiltered.length,
                items: importableFiltered.map(toItem),
              }),
            }, importing ? t("importing") : t("import.filter", { n: importableFiltered.length })),
            React.createElement("button", {
              style: {
                ...style.toolBtn, flex: "none", fontWeight: 600,
                opacity: refreshableFiltered.length === 0 || importing || !stream.done ? 0.55 : 1,
              },
              disabled: refreshableFiltered.length === 0 || importing || !stream.done,
              title: t("refresh.imported.title"),
              onClick: () => setConfirmRefreshImported({
                n: refreshableFiltered.length,
                items: refreshableFiltered.map(toItem),
              }),
            }, importing ? t("importing") : t("refresh.imported", { n: refreshableFiltered.length })),
            React.createElement("button", {
              style: {
                ...style.toolBtn, flex: "none", fontWeight: 600,
                opacity: importableAll.length === 0 || importing || !stream.done ? 0.55 : 1,
                borderColor: colors.accent, color: colors.accent,
              },
              disabled: importableAll.length === 0 || importing || !stream.done,
              title: t("import.all.title"),
              onClick: () => doImport(importableAll.map(toItem)),
            }, importing ? t("importing") : t("import.all", { n: importableAll.length }))),
      ...
      if (embedded) {
        return React.createElement("div", { style: { display: "flex", flexDirection: "column", minHeight: 0, flex: 1, position: "relative" } },
          body, filterImportDialog, refreshImportDialog);
      }
      return React.createElement("div", { style: style.overlay, onClick: onClose },
        React.createElement("div", { style: { ...style.panel, position: "relative" }, onClick: (e) => e.stopPropagation() },
          React.createElement("div", { style: style.header },
            React.createElement("span", { style: style.title }, t("panel.title")),
            React.createElement("button", { style: style.close, onClick: onClose, title: t("close") }, "✕")),
          body, filterImportDialog, refreshImportDialog));
     ...
     *    正上方独占一行、左缘与宽度对齐，形成并行两行；
     * 3) 容器 nowrap 且无占用者 → 行内自适应：row 容器 flex: 1 1 auto 与同
     *    槽其它入口共享一行（issue #31）；column 容器（usage-stats 纵排）
     *    保持全宽、高度随内容（issue #25）。
     ...
     * `flex:none; width:256px`）、内置插件市场 launcher（.dshMarketLauncher，
     * `flex:none; width:calc(100% + 4px)`）、@kenz1117/dsh-ui-usage-billing 的
     * 计费卡（[data-testid='billing-trigger']，`flex:auto; width:100%`）与
     * dsh-cost-meter 的侧栏费用卡（.cm-footer-stack，`flex-direction:column;
     * width:100%`）都会占满不换行的 footer flex 行，把同槽其它条目挤成窄条/竖排
     * 文字。返回最靠上的一个可见占用者的视口坐标（{top,left,width,height}），
     * 无则 null —— 用于把本按钮浮到其上方独占一行、左缘与宽度对齐，与占用者
     * 形成并行两行布局。 */
    const footerOccupantRect = () => {
      if (typeof document === "undefined") return null;
      let best = null;
      for (const sel of ["[data-cordis-badge]", ".dshMarketLauncher", "[data-testid='billing-trigger']", ".cm-footer-stack"]) {
    ...
    /** 槽容器的 flex 布局：{ wraps, dir }（marker 父元素的 flex-wrap /
     * flex-direction 计算值，marker 缺失时 { wraps:false, dir:null }）。
     * wraps=true（如 dsh-tokenledger 注入 flex-wrap:wrap）→ 行内整宽条目各
     * 自成行、order 决定堆叠，本按钮落回行内即可、无需浮层；dir 为 column
     * （如 dsh-usage-stats 强制纵排）→ 非浮动样式须保持高度随内容（issue
     * #25）。经 data-slot 锚点的父元素判定，不依赖 CSS-module 哈希类。 */
    const footerLayout = () => {
    ...
    function ImportButton({ wide }) {
      ...
      // nowrap 且无占用者 → 行内条目：row 容器 flex: 1 1 auto 与同槽其它入口
      // 共享一行（issue #31），column 容器全宽、高度随内容（issue #25）。
      // MutationObserver（观察根元素，含 head 里样式表的增删）+ resize 跟踪
      // 布局与占用者变化；状态更新做等值比较防止观察自身 DOM 变化造成重渲染循环。
      ...
      // row 方向且未 wrap 的行内容器（marker 缺失 / 其它插件改纵排 / wrap 时
      // 均为 false，按全宽条目兜底）；floating 分支不关心方向。
      ...
      // 16×16 图标；rail 态 36×36、圆角 50%、单图标 18px 居中。颜色/悬停走
      ...
      const triggerStyle = floating
        ...
        : {
          // 非浮动行内样式按容器 flex 布局三分派（rail 态恒为 36×36 圆钮）：
          // - row 容器（默认 256px footer 行、nowrap、无占用者）：flex: 1 1
          //   auto + width: auto + min-width: 0，与同槽其它入口（如 dsh-web-all
          //   的「检查更新/远程访问」）共享一行、按比例分配宽度（issue #31）。
          //   早期用 width: 100% + flex: 0 0 auto 独占整行，把同槽条目挤出
          //   280px 侧栏被 overflow: hidden 裁剪；仅本插件一个入口时 flex-grow
          //   仍撑满整行，视觉与 width: 100% 一致。
          // - column 容器（如 dsh-usage-stats 强制纵排，issue #25）：flex-basis
          //   沿主轴=高度解析，grow: 1 会把按钮整高拉伸压住同槽按钮 → 保持
          //   flex: 0 0 auto + width: 100%：占满宽、高度随内容。
          // - wrap 容器（如 dsh-tokenledger 注入 flex-wrap:wrap）：width: 100%
          //   让各整宽条目各自成行、order 决定堆叠；auto 宽 + grow 的条目会挤
          //   到同一行破坏堆叠 → 同样保持 flex: 0 0 auto + width: 100%。
          ...baseStyle,
          ...(rail || !rowFree
      ...
      return React.createElement(React.Fragment, null,
        React.createElement("button", {
          ...
          onClick: () => {
            ...
            //（path seed 走 openTab 的 CONTENT open 路径，自动 expand panel）；
            // 未安装 → 回退自绘 ShellPanel。
            if (betterSidebarService) {
              try {
                betterSidebarService.openTab({ type: IMPORT_TAB_TYPE, path: IMPORT_TAB_TYPE });
                return;
              } catch (err) {
          ...
          React.createElement(LogoIcon, { size: rail ? 18 : 16 }),
