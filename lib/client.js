...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
      ...
      useEffect(() => {
        ...
        (async () => {
          ...
          while (!cancelled && !done && !failed) {
            ...
            setStream({ done, cursor: after, total: typeof data.total === "number" ? data.total : 0, started: true });
    ...
    function apply(ctx) {
      ...
      // 「导入系统提示词」开关（默认关）。该槽由 ui-settings-plugins 声明，而
      // ui-settings-plugins 依赖 settingsScope（settingsScope 又依赖 connection/remote），
      // 晚于本插件 apply 期。settingsScope 是可选服务（dsh-client-ui-settings），缺席时
      // 不注册；slots.inject 惰性挂到该槽被 ui-settings-plugins 声明时，槽不存在（无
      // 设置页的 profile）则回调永不执行、不报错。必须在回调内 resolve settingsScope——
      // 槽声明时它已就绪；apply 期 ctx.get('settingsScope') 拿到 undefined 会让整段
      // 跳过、TAB 永不注册（设置页「导入系统提示词」按钮打不开的根因）。label 用 thunk
      // 跟随语言切换（同官方 plugins 分区 tab 的写法）。
