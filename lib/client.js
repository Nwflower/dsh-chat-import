...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
     ...
     *    容器无法定位时才回退 fixed 浮层锚到占用者正上方（仅宽态；rail 态同槽
     *    占用者与按钮同处 56px 纵栏，浮层必压其行——issue #39，改走行内圆钮）；
     ...
     */
    /** 检测 footer 行内把本插件按钮挤到「显示不下」的同槽条目。
     *
     * 不再靠选择器白名单识别占用者：整行条目的来源持续变多（cordis 徽标 → 插件市场
     * launcher → usage-billing 计费卡 → cost-meter 费用卡 → 内置「手机连接」…），每
     * 出现一个来源就补一条选择器永远追不上，且 CSS-module 哈希类名跨构建不稳定
     * （#31 / #35 / #39 / #43 是同一类问题的四次复发）。改按**布局事实**判定，与
     * 「它是谁」无关（判定逻辑与 lib/footer-layout.mjs 同步）：
     *   ① 本插件的槽元素内容被裁（scrollWidth > clientWidth）——「被挤成窄条 / 文字
     *      截断」的直接症状，占用者即使已被 flex 压扁也必然留下这个痕迹；
     *   ② 同槽有渲染宽度≈容器宽的可见条目（未被挤压的整宽条目）。
     * 半宽入口共享一行（#31 的预期行为）两个信号都不命中，行为不变。
     * 保留的选择器只作历史兜底（占用者被宿主包了一层、或几何判定被取整吞掉），
     * **新增来源无需再补**。
     * 返回最靠上的一个占用条目的视口坐标（fixed 浮层兜底的定位锚点），无则 null。 */
    const FOOTER_OCCUPANT_LEGACY = ["[data-cordis-badge]", ".dshMarketLauncher", "[data-testid='billing-trigger']", ".cm-footer-stack"];
    // 与 lib/footer-layout.mjs 同步（浏览器 bundle 不 import 模块，各存一份）
    ...
    const footerOccupantRect = () => {
      ...
      const consider = (el) => {
        ...
        const occupant = isFooterOccupant({
          visible: cs.display !== "none" && cs.visibility !== "hidden" && cs.opacity !== "0",
          position: cs.position,
          width: r.width,
          height: r.height,
        }, context);
        if (occupant && (!best || r.top < best.top)) best = { top: r.top, left: r.left, width: r.width, height: r.height };
      };
      ...
      if (best) return best;
      // 已被裁但没有任何可锚的兄弟（占用者也在容器之外且被压扁）→ 锚到本按钮自身，
      // 让 fixed 浮层落在同一区域；容器一旦被换成 wrap 该项即不再命中（见下方 effect）
    ...
    function ImportButton({ wide }) {
      ...
      useEffect(() => {
        ...
        const check = () => {
          ...
          // 检测到占用者（任何把本按钮挤到显示不下的同槽条目，见 footerOccupantRect）
          // 时给 footerActions 换行，让本按钮以整宽行落到占用者下方、与「插件/设置」
          // 纵向堆叠，避免 fixed 浮层压住会话列表末尾行
          // （列表长时重叠）；无占用者不动容器（保持 issue #31 共享一行），已 wrap
          // （tokenledger 等注入）不覆盖。仅在宽态（data-dsh-sidebar-wide="true"）换行：
          // rail 容器 width:auto，强行 wrap 会影响 56px 纵栏里图标堆叠的居中；rail 态
          // 同时也不浮层（见下方 floating 判定），否则同槽占用者就在本按钮同一纵栏，
          // 浮层会正压其行（issue #39）。写入 inline style 会触发 MutationObserver 再跑
      ...
      // rail 态恒不浮层：56px 纵栏里同槽占用者（usage-stats 的 billing-trigger 等）
      // 与本按钮同列，按占用者 rect 定位的 fixed 浮层会正压其「用量/余额」行
      // （issue #39）——rail 走行内 36×36 圆钮分支，与无占用者时同款。
