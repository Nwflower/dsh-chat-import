...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function DiscoveryPanel({ onClose, embedded }) {
      ...
      const [confirmFilterImport, setConfirmFilterImport] = useState(null); // { n }
      ...
      const importableAll = stream.done
        ? items.filter((s) => s.importStatus !== "imported" && s.importStatus !== "archived")
        : [];
      const importableFiltered = stream.done
        ? filteredItems.filter((s) => s.importStatus !== "imported" && s.importStatus !== "archived")
        : [];
