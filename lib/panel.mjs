...
export async function importDiscoveryItem(ctx, format, sourcePath, sessionIds, { force, replace, budget, budgetSource }) {
  ...
  const importSingle = io.file
    || ((c, t, a) => importTranscript(c, t, a, spec.convert, { registryDir: reg.dir, fingerprintKeys: reg.fingerprintKeys || [], sourceLabel: spec.sourceLabel }))
  ...
  const args = { path: sourcePath, force: force === true, replace: replace === true, budget, budgetSource }
