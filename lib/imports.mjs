 ...
 * 无标记（旧版本导入）或读不到日志返回 null。readable 区分「日志可读但无标记」
 * 与「日志读不到」（后者是 retract 后工件被删的幽灵会话信号，issue #22）：
...
async function sessionOwnerPath(ctx, dshId) {
  const sp = ctx.get('sessionPersistence')
  if (!sp || typeof sp.inspect !== 'function') return { owner: null, readable: null }
  try {
    const info = await sp.inspect(dshId)
    ...
    if (first && first.type === 'session/imported' && first.data && typeof first.data.sourcePath === 'string') {
      return { owner: first.data.sourcePath, readable: true }
    }
    return { owner: null, readable: true }
  } catch {
    // 读不到日志（工件已删 / 后端瞬断）：按无标记 + 不可读处理
    return { owner: null, readable: false }
  }
}
...
// __action 字段。
async function decideItem(ctx, { known, converted, args, persisted, sourcePath, archivedIds }) {
  ...
  if (known && typeof known.dshId === 'string' && typeof known.turns === 'number') {
    ...
    if (args.replace === true) {
      return {
        ...
        __events: events,
        __itemRecord: itemRecord(targetId, turns.length, events.length),
      }
    }
  ...
  if (persisted.has(meta.id)) {
    ...
    if (owner === null && readable === false && !archived.has(meta.id)) {
...
export async function decideSingle(ctx, { known, converted, stat, args, fingerprint, persisted, sourcePath, budget, archivedIds }) {
...
export async function decideMulti(ctx, { known, items, stat, args, fingerprint, persisted, sourcePath, subTable, budget, archivedIds }) {
